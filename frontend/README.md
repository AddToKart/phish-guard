...
This launches the Vite dev server and `python ../backend/run_server.py` concurrently. Make sure you have the backend virtual environment prepared first so that Python/uvicorn are available.

- **Backend only via npm:**
...
- Modern glassmorphism UI with gradient backdrop
- Live integration with the FastAPI backend (`/analyze` endpoint)
- Feature vector visualization (signal weights & evidence)
- Sample dataset to explore the experience without making a request
